...
fn siliconMark(p: vec2f) -> f32 {
  let q = rotate(p, -0.14);
  let plate = sdRoundedBox(q, vec2f(0.20, 0.17), 0.03);
  let core = sdRoundedBox(q, vec2f(0.11, 0.08), 0.015);
  var mark = opSmoothSub(plate, core, 0.01);
  let routeA = sdRoundedBox(q - vec2f(0.0, 0.125), vec2f(0.035, 0.015), 0.006);
  let routeB = sdRoundedBox(q - vec2f(0.0, -0.125), vec2f(0.035, 0.015), 0.006);
  mark = opSmoothUnion(mark, routeA, 0.006);
  mark = opSmoothUnion(mark, routeB, 0.006);
  return mark;
}
...
fn fs_main(@location(0) uv: vec2f) -> @location(0) vec4f {
  ...
  let red = vec3f(0.58, 0.13, 0.12);
  let copper = vec3f(0.62, 0.38, 0.21);
  let paper = vec3f(0.96, 0.91, 0.82);
  let leftClear = smoothstep(0.34, 0.62, uv.x);
  var a = fill * 0.08 + line * 0.28 + glow * 0.08 + traces * 0.32 + spark * 0.06;
  a *= 0.55 * leftClear;
  a = clamp(a, 0.0, 0.34);
  let metal = mix(red, copper, spark * 0.45);
  let col = mix(metal, paper, line * 0.05);
